...
from wdm.analysis.selector import (
    _apply_hard_filters, _rank_and_auto_keep, _resolve_psi_knobs,
)


def _toy_df():
    """Minimal per-feature frame with the columns _apply_hard_filters /
    _rank_and_auto_keep actually touch. PSI spans below and above the
...
def test_soft_mode_does_not_drop_high_psi():
    df = _apply_hard_filters(_toy_df(), _cfg(psi_mode="soft"))
...
def test_hard_mode_drops_high_psi():
    df = _apply_hard_filters(_toy_df(), _cfg(psi_mode="hard"))
    high = df[df["feature"] == "high_psi_good"].iloc[0]
...
def test_off_mode_matches_soft_for_hard_filters():
    ...
    df_soft = _apply_hard_filters(_toy_df(), _cfg(psi_mode="soft"))
    df_off = _apply_hard_filters(_toy_df(), _cfg(psi_mode="off"))
    assert list(df_soft["_hard_drop_reason"]) == list(df_off["_hard_drop_reason"])


# --- _rank_and_auto_keep (rank_score term) ----------------------------------

def test_off_mode_zeros_psi_contribution_to_rank_score():
    base = _apply_hard_filters(_toy_df(), _cfg(psi_mode="off"))
    base["corr_cluster"] = -1
    base["family_kept"] = True
    base["group_kept"] = True
    ranked = _rank_and_auto_keep(base, _cfg(psi_mode="off"))
...
def test_soft_mode_penalizes_high_psi_in_rank_score():
    base = _apply_hard_filters(_toy_df(), _cfg(psi_mode="soft"))
    base["corr_cluster"] = -1
    base["family_kept"] = True
    base["group_kept"] = True
    ranked = _rank_and_auto_keep(base, _cfg(psi_mode="soft"))
    r_low = ranked[ranked["feature"] == "low_psi_good"].iloc[0]["rank_score"]
    r_high = ranked[ranked["feature"] == "high_psi_good"].iloc[0]["rank_score"]
